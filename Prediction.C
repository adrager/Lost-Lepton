...
void Prediction::SlaveBegin(TTree * /*tree*/)
{
	...
	MuMTWHT_ = (TH1F*) EffInputFolder->Get("MuMTWHT1D");
// 	MuMTWNJets_ = (TH1F*) EffInputFolder->Get("MuMTWNJets1D");
	...
	MuDiLepEffMTWAppliedNJets_ = (TH1F*) EffInputFolder->Get("DiLepNJets1D");
        MuDiLepEffBTags_ = (TH1F*) EffInputFolder->Get("MuDiLepBTag1D");
        ElecDiLepEffBTags_ = (TH1F*) EffInputFolder->Get("ElecDiLepBTag1D");
	...
	ElecMTWHT_ = (TH1F*) EffInputFolder->Get("ElecMTWHT1D");
// 	ElecMTWNJets_ = (TH1F*) EffInputFolder->Get("ElecMTWNJets1D");
	ElecDiLepContributionMTWAppliedNJets_ = (TH1F*) EffInputFolder->Get("ElecDiLepContributionMTWNJets1D");
        ElecDiLepEffMTWAppliedNJets_ = (TH1F*) EffInputFolder->Get("DiLepNJets1D");
	...
	tPrediction_->Branch("purityCorrectedWeight",&purityCorrectedWeight_,"purityCorrectedWeight/F");
...
Bool_t Prediction::Process(Long64_t entry)
{
	...
	//      if((GenMuNum+GenElecNum)!=1) return kTRUE;
			 ...
			 if(selectedIDIsoMuonsNum==1 && selectedIDIsoElectronsNum==0)
			 {
				 ...
				 if(!UseUpdatedTEfficiencies_) muMTWEff_ = getEff(MuMTWHT_,HT);
			 ...
			 {
// 				 std::cout<<"MuIsoWeigth computed right way\n";
			 ...
			 if(!UseUpdatedTEfficiencies_) 
			 {
				 ...
				 totalWeight_ = elecTotalWeight_ + muTotalWeight_;
                                 if(!UseUpdatedTEfficiencies_) muDiLepEffMTWAppliedEff_ = getEff(MuDiLepEffBTags_,BTags);
...
// 			 totalWeightDiLep_ = totalWeight_ + (1-muDiLepContributionMTWAppliedEff_) * mtwCorrectedWeight_ * (1-muDiLepEffMTWAppliedEff_)/muDiLepEffMTWAppliedEff_;
			 totalWeightDiLep_ = totalWeight_ + mtwCorrectedWeight_ * (1-muDiLepEffMTWAppliedEff_);
			 DiLepContribution+= muDiLepContributionMTWAppliedEff_;
			 DiLepEff_+= muDiLepEffMTWAppliedEff_;
			 ...
			 if(!UseUpdatedTEfficiencies_) elecMTWEff_ = getEff(ElecMTWHT_,HT);
			 ...
			 muTotalWeight_ = muIsoWeight_ + muRecoWeight_ + muAccWeight_;
			 totalWeight_ = elecTotalWeight_ + muTotalWeight_;
                         if(!UseUpdatedTEfficiencies_) elecDiLepEffMTWAppliedEff_ = getEff(ElecDiLepEffBTags_,BTags);
...
// 			 totalWeightDiLep_ = totalWeight_ + (1-elecDiLepContributionMTWAppliedEff_) * mtwCorrectedWeight_ * (1-elecDiLepEffMTWAppliedEff_)/elecDiLepEffMTWAppliedEff_;
			 totalWeightDiLep_ = totalWeight_ + mtwCorrectedWeight_ * (1-elecDiLepEffMTWAppliedEff_);
			 DiLepContribution+= elecDiLepContributionMTWAppliedEff_;
			 DiLepEff_+= elecDiLepEffMTWAppliedEff_;
			 totalWeightDiLepIsoTrackReduced_ = totalWeightDiLep_ * (1 - expectationReductionIsoTrackEff_);
			 IsoTrackReduction_ = totalWeightDiLep_ *expectationReductionIsoTrackEff_;
			 totalWeightDiLepIsoMuTrackReduced_ = totalWeightDiLep_ * (1 - expectationReductionMuIsoTrackEff_);
			 IsoMuTrackReduction_ = totalWeightDiLep_ *expectationReductionMuIsoTrackEff_;
			 ...
			 if((selectedIDIsoMuonsNum+selectedIDIsoElectronsNum)==1)
			 {
				 NoDiLepWeight_+= totalWeight_;
				 WithDiLepWeight_+= totalWeightDiLep_;
				 events_++;
				 averageWeight_+=mtwCorrectedWeight_;

			 }
			 //      if(selectedIDIsoMuonsNum==0 && selectedIDIsoElectronsNum==0 && !IsolatedTracksMuMatched_ && !IsolatedTracksElecMatched_) return kTRUE;
			 tPrediction_->Fill();
			 // 	std::cout<<"--------------------------------------------------------------\n";
 			 if(selectedIDIsoMuonsNum==1 && selectedIDIsoElectronsNum==0 && false)
 			 {
 				 std::cout<<"mu control-sample\n";
 				 std::cout<<"mtw: "<<mtw<<"\n";
 				 std::cout<<"Weight: "<<Weight<<"\n";
 				 std::cout<<"PurityEff: "<<muPurityCorrection_<<"\n";
 				 std::cout<<"PurityCorrectedWeight: "<<purityCorrectedWeight_<<" correction: "<<purityCorrectedWeight_/Weight<<"\n";
 				 std::cout<<"MTWEff: "<<muMTWEff_<<"\n";
 				 std::cout<<"MTWCorrectedWeight:    "<<mtwCorrectedWeight_<<" correction: "<<mtwCorrectedWeight_/Weight<<"\n";
 				 std::cout<<"DiLepEff: "<<muDiLepContributionMTWAppliedEff_<<"\n";
 				 std::cout<<"MTWCorrectedWeight:    "<<mtwDiLepCorrectedWeight_<<" correction: "<<mtwDiLepCorrectedWeight_/Weight<<"\n";
                                 std::cout<<"DiLepBkgEstimation: "<<muDiLepEffMTWAppliedEff_<<"\n";
                                 std::cout<<"MTWCorrectedWeight:    "<<(1-muDiLepContributionMTWAppliedEff_) * mtwCorrectedWeight_ * (1-muDiLepEffMTWAppliedEff_)/muDiLepEffMTWAppliedEff_<<" correction: "<<(1-muDiLepContributionMTWAppliedEff_) * mtwCorrectedWeight_ * (1-muDiLepEffMTWAppliedEff_)/muDiLepEffMTWAppliedEff_/totalWeight_<<"\n";
 				 
 			 }
// 			 else if(selectedIDIsoMuonsNum==0 && selectedIDIsoElectronsNum==1)
// 			 {
// 				 std::cout<<"elec control-sample\n";
// 				 std::cout<<"mtw: "<<mtw<<"\n";
// 				 std::cout<<"Weight: "<<Weight<<"\n";
// 				 std::cout<<"PurityEff: "<<elecPurityCorrection_<<"\n";
// 				 std::cout<<"PurityCorrectedWeight: "<<purityCorrectedWeight_<<" correction: "<<purityCorrectedWeight_/Weight<<"\n";
// 				 std::cout<<"MTWEff: "<<elecMTWEff_<<"\n";
// 				 std::cout<<"MTWCorrectedWeight:    "<<mtwCorrectedWeight_<<" correction: "<<mtwCorrectedWeight_/Weight<<"\n";
// 				 std::cout<<"DiLepEff: "<<elecDiLepContributionMTWAppliedEff_<<"\n";
// 				 std::cout<<"MTWCorrectedWeight:    "<<mtwDiLepCorrectedWeight_<<" correction: "<<mtwDiLepCorrectedWeight_/Weight<<"\n";
// 			 }
...
void Prediction::Terminate()
{
	...
	std::cout<<"Total Weight: "<<NoDiLepWeight_<<" (withoutDiLep) with: "<<WithDiLepWeight_<<" additionalEvents: "<<WithDiLepWeight_-NoDiLepWeight_<<"\n";
	std::cout<<"Average DiLepContribution: "<<DiLepContribution/events_<<" avarage dilepeff "<< DiLepEff_/events_<<" prediction: "<<averageWeight_ *(1-DiLepContribution/events_)*(1-DiLepEff_/events_)/(DiLepEff_/events_)<<"\n";
...
double EffValue::Up()
{
	if ((Eff_+EffUp_)>0.9999)
	{
		std::cout<<"EffValue::Warning EffUp would be: "<<Eff_+EffUp_<<" setting to 0.9999\n";
		return 0.9999;
	}
	else if ((Eff_+EffUp_)<0.000001)
	{
		std::cout<<"EffValue::Warning EffUp would be: "<<Eff_+EffUp_<<" setting to 0.000001\n";
		return 0.000001;
	}
	else return Eff_+EffUp_;
			 return 0;
}
double EffValue::Down()
{
	if ((Eff_-EffDown_)>0.9999)
	{
		std::cout<<"EffValue::Warning EffDown would be: "<<Eff_+EffDown_<<" setting to 0.9999\n";
		return 0.9999;
	}
	else if ((Eff_-EffDown_)<0.000001)
	{
		std::cout<<"EffValue::Warning EffDown would be: "<<Eff_+EffDown_<<" setting to 0.000001\n";
		return 0.000001;
	}
	else return Eff_-EffDown_;
			 return 0;
}
...
double EffValue::Eff()
{
	if ((Eff_)>0.9999)
	{
		std::cout<<"EffValue::Warning Eff would be: "<<Eff_<<" setting to 0.9999\n";
		return 0.9999;
	}
	else if ((Eff_)<0.000001)
	{
		std::cout<<"EffValue::Warning Eff would be: "<<Eff_<<" setting to 0.000001\n";
		return 0.000001;
	}
	else return Eff_;
			 return 0;

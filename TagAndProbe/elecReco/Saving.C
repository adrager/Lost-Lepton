...
void Saving::SlaveBegin(TTree * /*tree*/)
{
   ...
   TString option = GetOption();
	 ...
	 tExpectation_ = new TTree("TagAndProbeElecReco","");
...
{
	TFile *outPutFile = new TFile("TagAndProbeEff_ElecReco.root","RECREATE"); 
	outPutFile->cd();
	outPutFile->mkdir("ElecReco");
	TDirectory *dmuonEffs = (TDirectory*)outPutFile->Get("ElecReco");
	dmuonEffs->cd();
	tExpectation_->Write();
	outPutFile->Close();

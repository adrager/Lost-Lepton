...
// uncertainties
const bool DoUncertainties_=true;
...
const double NonClosure_= 20.0; // percentage relative to full precdiction
...
// quick study for inclusive search meeting
const bool UseHTIsoEff_=false;
const bool UseNJetsIsoEff_=false;
// use RelptDeltaR for mu elec iso only. no effect if not selecting default muIso activity pt
const bool UseDeltaRRelPT=false; // this should actually work 30 July
class EffValue
{
public:
	EffValue()
	...
	EffValue(double Eff1, double EffDown, double EffUp)
	{
		Eff_=Eff1;
		EffDown_=EffDown;
		EffUp_=EffUp;
	...
	void Set(double Eff1, double EffDown, double EffUp)
	{
		Eff_=Eff1;
		EffDown_=EffDown;
		EffUp_=EffUp;
...
public :
	...
	unsigned int UnMatchedIsoTracks(double eta, double phi, double pt);
	...
	UShort_t searchBin_, Bin_, AdditionalTracks_;
	
	// study using truth information
	...
	// studies for di lep correction
	int events_;
	double averageWeight_;
	double NoDiLepWeight_, WithDiLepWeight_,DiLepContribution, DiLepEff_;
	
	// Effiecineices
	//    TH2F *MuMTWPTActivity_;
	TH1F *MuMTWHT_;
	TH1F *MuDiLepContributionMTWAppliedNJets_;
        TH1F *MuDiLepEffBTags_ ,   *ElecDiLepEffBTags_;
	TH2F *MuIsoPTActivity_, *MuIsoPTActivityTAPMC_, *MuIsoDeltaRRelPTJet_;
	...
	TH2F *ElecIsoPTActivity_, *ElecIsoPTActivityTAPMC_, *ElecIsoDeltaRRelPTJet_;
	TH1F *MuIsoHT_;
	TH1F *MuIsoNJets_;
	TH1F *ElecIsoHT_;
	TH1F *ElecIsoNJets_;
	...
	Float_t muIsoEffUp_, muIsoEffDown_, muRecoEffUp_, muRecoEffDown_, muAccEffUp_, muAccEffDown_;
	Float_t elecIsoEffUp_, elecIsoEffDown_, elecRecoEffUp_, elecRecoEffDown_, elecAccEffUp_, elecAccEffDown_;
	
	Float_t muIsoRel_, muRecoRel_, muAccRel_, elecIsoRel_, elecRecoRel_, elecAccRel_;
	
	Float_t mtwDown_, mtwUp_, diLepDown_, diLepUp_;
	Float_t nonClosureUp_, nonClosureDown_; 
